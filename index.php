...
$public_events = [];
$search_keyword = isset($_GET['search']) ? trim($_GET['search']) : ''; // Ambil kata kunci pencarian
...
try {
    ...
    $sql .= " ORDER BY tanggal_mulai ASC LIMIT 9"; // Tampilkan hingga 9 event
...
<header class="bg-white sticky top-0 z-50 shadow-md">
    <div class="container mx-auto px-6 py-4 flex justify-between items-center">
        <a href="index.php" class="text-2xl font-bold text-green-600">Agendain</a>
        <div class="flex items-center space-x-6">
            ...
            <div class="flex items-center space-x-4">
                <a href="login.php" class="text-gray-600 hover:text-green-600 transition font-medium">Login Panitia</a>
                <a href="register.php" class="bg-green-500 text-white font-semibold px-5 py-2 rounded-lg hover:bg-green-600 transition duration-300">Daftar Panitia</a>
            </div>
        </div>
    </div>
</header>
...
<main>
    <section class="bg-white">
        <div class="container mx-auto px-6 py-20 grid md:grid-cols-2 gap-12 items-center">
            ...
            <div class="text-center md:text-left">
                ...
                <p class="mt-4 text-lg text-gray-600">Dari pendaftaran peserta hingga distribusi sertifikat, <span class="font-semibold text-gray-800">Agendain</span> membuat semuanya terorganisir dalam satu platform.</p>
                <div class="mt-8 flex justify-center md:justify-start gap-4">
                    <a href="register.php" class="bg-green-500 text-white font-semibold px-8 py-3 rounded-lg hover:bg-green-600 transition duration-300">Daftar Gratis</a>
                    <a href="#fitur" class="bg-white border border-gray-300 text-gray-700 font-semibold px-8 py-3 rounded-lg hover:bg-gray-100 transition duration-300">Lihat Fitur</a>
                </div>
            </div>
        </div>
    </section>
    ...
    <section id="event-terbaru" class="py-20 bg-white">
        <div class="container mx-auto px-6">
            ...
            <div class="max-w-xl mx-auto mb-12">
                <form action="index.php" method="GET" class="flex shadow-md rounded-lg overflow-hidden">
                    <input type="text" name="search" class="flex-grow px-6 py-3 border-none focus:ring-2 focus:ring-green-500 outline-none" 
                           placeholder="Cari event..." value="<?php echo htmlspecialchars($search_keyword); ?>">
                    <button type="submit" class="bg-green-500 text-white px-8 py-3 font-semibold hover:bg-green-600 transition">Cari</button>
                    <?php if (!empty($search_keyword)): ?>
                        <a href="index.php" class="bg-gray-200 text-gray-700 px-6 py-3 font-semibold hover:bg-gray-300 transition flex items-center">Reset</a>
                    <?php endif; ?>
                </form>
            </div>
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                <?php if (empty($public_events)): ?>
                    <div class="col-span-full text-center py-12">
                        <?php if (!empty($search_keyword)): ?>
                            <p class="text-gray-500 text-xl">Tidak ada event yang cocok dengan "<strong><?php echo htmlspecialchars($search_keyword); ?></strong>".</p>
                        <?php else: ?>
                    ...
                    <?php foreach ($public_events as $event): ?>
                        <div class="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col transition-all duration-300 ease-in-out hover:shadow-2xl">
                            ...
                            <div class="p-6 flex flex-col flex-grow">
                                ...
                                <p class="text-sm text-gray-500 mb-1">📅 <?php echo date('d F Y, H:i', strtotime($event['tanggal_mulai'])); ?> WIB</p>
                                <p class="text-gray-600 text-sm mb-4 flex-grow line-clamp-3"><?php echo htmlspecialchars($event['deskripsi']); ?></p>
    ...
    <section id="cta" class="py-20 bg-green-50">
        <div class="container mx-auto px-6 text-center">
            ...
            <p class="mt-3 text-lg text-gray-600 max-w-2xl mx-auto">Coba <span class="font-semibold text-gray-800">Agendain</span> sekarang dan rasakan kemudahan mengelola acara dari awal hingga akhir.</p>
            <div class="mt-8">
                <a href="register.php" class="bg-green-500 text-white font-semibold px-8 py-3 rounded-lg hover:bg-green-600 transition duration-300 shadow-lg">Daftarkan Organisasi Anda (Gratis)</a>
            </div>
        </div>
    </section>
</main>
...
<footer id="kontak" class="bg-white text-center">
    ...
    <div class="py-4">
        <div class="container mx-auto px-6 text-sm text-gray-500"><p>Platform Manajemen Event</p></div>
    </div>
</footer>
